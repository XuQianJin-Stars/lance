...
//!
//! Run:
//!   cargo test -p lance-io --features goosefs --test goosefs_integration -- --ignored --nocapture --test-threads=1
#![cfg(feature = "goosefs")]
...
async fn test_diag_lance_io_write_modes() {
    ...
    println!("[DIAG] Creating ObjectStore at: {}", root);
    ...
    match object_store
        ...
        .await
    {
        Ok(_) => println!("[DIAG] Write succeeded! ✅"),
        Err(e) => {
            println!("[DIAG] Write FAILED: {:?}", e);
            println!("[DIAG] Error source: {:?}", std::error::Error::source(&e));
            return;
        }
    }

    println!("[DIAG] Reading test_file.txt...");
    match object_store.inner.get(&test_path).await {
        Ok(result) => {
            let bytes = result.bytes().await.unwrap();
            let content = String::from_utf8_lossy(&bytes);
            println!("[DIAG] Read content: '{}' ({} bytes)", content, bytes.len());
            assert_eq!(bytes, test_data);
        }
        Err(e) => println!("[DIAG] Read FAILED: {:?}", e),
    }

    // Test 2: PutMode::Create (if_not_exists)
    println!("[DIAG] Writing with PutMode::Create (if_not_exists)...");
    match object_store
        .inner
        .put_opts(
        ...
        .await
    {
        Ok(_) => println!("[DIAG] PutMode::Create succeeded! ✅"),
        Err(e) => {
            println!("[DIAG] PutMode::Create FAILED: {:?}", e);
        }
    }

    // Test 3: rename_if_not_exists
    println!("[DIAG] Testing rename_if_not_exists...");
    ...
    {
        Ok(_) => {
            println!("[DIAG] Tmp file written ✅");
            match object_store
                .inner
                .rename_if_not_exists(&tmp_path, &dest_path)
                .await
            {
                Ok(_) => println!("[DIAG] rename_if_not_exists succeeded! ✅"),
                Err(e) => println!("[DIAG] rename_if_not_exists FAILED: {:?}", e),
            }
        }
        Err(e) => println!("[DIAG] Tmp file write FAILED: {:?}", e),
    }

    println!("[DIAG] lance-io direct write test complete ✅");
}
